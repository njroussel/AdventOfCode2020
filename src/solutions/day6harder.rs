...
impl GroupAnswers {
    pub fn from_str(input_str: &String) -> io::Result<GroupAnswers> {
        ...
        let person_answers: Vec<&str> = input_str.trim().split(' ').collect();
        let mut person_surveys: Vec<Vec<bool>> =
            person_answers.iter().map(|_| vec![false; 26]).collect();

        person_answers.iter().enumerate().for_each(|(i, answers)| {
            answers
                .chars()
                .for_each(|question_char| match question_char {
                    ' ' => (),
                    any_char => person_surveys[i][((any_char as i32) - 97) as usize] = true,
                })
        });

        out.answered_questions = (0..26)
            .map(|i| {
                person_surveys
                    .iter()
                    .fold(true, |and, answers| and && answers[i])
            })
            .collect();
